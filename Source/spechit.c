...
object *monster;
{
	...
	if (monster->m_flags & IMITATES) {
		...
		if (!blind) {
			...
			check_message();
#ifdef JAPAN
			sprintf(msg, "おっと、こいつは%sだ！",
#else
			sprintf(msg, "Wait, that's a %s!",
#endif
						mon_name(monster));
			message(msg, 1);
		}
