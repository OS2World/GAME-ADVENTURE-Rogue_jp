...
check_up()
{
	if (!wizard) {
		...
		if (!has_amulet()) {
#ifdef JAPAN
			message("äKíiÇÕâΩé“Ç©ÇÃéËÇ…ÇÊÇ¡ÇƒÇ”Ç≥Ç™ÇÍÇƒÇ¢ÇÈÅB", 0);
#else
...
#ifdef JAPAN
	new_level_message = "à›ë‹ÇÇÀÇ∂êÿÇÁÇÍÇÈÇÊÇ§Ç»ãCï™Ç™ÇµÇΩÅB";
#else
...
#ifdef JAPAN
	sprintf(mbuf,"äÓñ{è„è∏ó¶: %d.%02d, é¿å¯è„è∏ó¶: %d.%02d (!: %d, V: %d)",
#else
