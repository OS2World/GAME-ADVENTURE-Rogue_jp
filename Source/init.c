...
char *argv[];
{
	...
	if (!score_only && !rest_file) {
		...
		putstr("やあ、");
		putstr(nick_name);
		putstr("。 少しだけ待っててくれるかい ...");
#else
		putstr("Hello ");
		putstr(nick_name);
		putstr(", just a moment while I dig the dungeon...");
#endif
		fflush(stdout);
	}
#endif
...
#ifdef JAPAN
		clean_up("ごめん！ 画面の大きさは最低２４×８０なんだ。");
#else
...
#ifndef ORIGINAL
	"通路の角で止まらずに進む",
	"ゲーム終了時に墓標を\225\134示する",
#else
	"ゲーム終了時に骸骨を\225\134示する",
#endif
#ifdef USE_COLOR
	"キャラクターをカラーで\225\134示する",
#endif
	"好きな果物",
	"セーブファイル名",
	"ニックネーム",
...
#endif
#ifdef USE_COLOR
	"キャラクターの\225\134示色マッピング",
#endif
#ifdef MSDOS
	"使用機種",
	"画面利用を開始するシーケンス",
	"画面利用を終了するシーケンス",
	"カー\203\134ルオン／オフのシーケンス",
#endif
	NULL
};
#else /*JAPAN*/
...
boolean add_blank, no_colon;
{
	...
	while ((*e) && (*e != ',')) {
#ifndef ORIGINAL
		if (*e > 0x80 && *e < 0xa0 || *e >= 0xe0 && *e < 0xf0) {
			e += 2;
			i += 2;
			continue;
		}
