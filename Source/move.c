...
#endif
#ifdef JAPAN
char *you_can_move_again = "ようやく体が自由になった。";
#else
...
short dirch, pickup;
{
	...
	if (being_held || bear_trap) {
		if (!(dungeon[row][col] & MONSTER)) {
			if (being_held) {
#ifdef JAPAN
				message("怪物につかまえられて、逃げられない。", 1);
#else
				message("You are being held", 1);
#endif
			} else {
#ifdef JAPAN
				message("熊のわなが、なかなかはずれない。", 0);
#else
...
boolean messages_only;
{
...
#ifdef JAPAN
		(void) strcpy(hunger_str, "飢餓");
		message("空腹のせいで力がなくなってきた。", 1);
#else
	...
	if (rogue.moves_left <= FAINT) {
		if (rogue.moves_left == FAINT) {
#ifdef JAPAN
			(void) strcpy(hunger_str, "瀕死");
			message("空腹で、もう死にそうだ。", 1);
#else
