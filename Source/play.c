...
char *help_message[] = {
#ifdef JAPAN
"?		ヘルプを\225\134示する		>   階段を降りる",
"/		画面の文字が\225\134すものを\225\134示	<   階段を登る",
...
"<CTRL>P 	最後のメッセージを再\225\134示する	T   よろいを脱ぐ",
"<CTRL>R 	画面を描き直す			P   指輪をはめる",
"<CTRL>D 	画面をファイルに書き出す	R   指輪をはずす",
"<ESC>		コマンドを中止する		d   持ちものを落とす",
"a		直前のコマンドを繰り返す	c   持ちものに呼び名を付ける",
"D		今まで何を発見したかを\225\134示	)   現在の武器を見る",
"S		ゲームをセーブして終了する	]   現在のよろいを見る",
"Q		ゲームを中止する		=   現在の指輪を見る",
"v		バージョン情報を\225\134示する	@   プレイヤー行を描き直す",
"!		シェルコマンドを実行する	o   オプションの\225\134示・設定",
"",
"＝スペースを押してください＝",
#else /*JAPAN*/
