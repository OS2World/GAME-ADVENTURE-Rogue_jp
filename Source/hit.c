...
boolean flame;
{
	...
	if (!fight_monster) {
		sprintf(hit_message + strlen(hit_message),
#ifdef JAPAN
		"%s%s‚Í–½’†‚µ‚½B ", (other? other:mn), (other? "":"‚ÌUŒ‚"));
#else
		"The %s hit", (other? other: mn));
#endif
		message(hit_message, 1);
		hit_message[0] = 0;
	}
...
boolean force_hit;
{
	...
	if (monster) {
		...
		if (!rand_percent(hit_chance)) {
			if (!fight_monster) {
#ifdef JAPAN
				sprintf(hit_message, "%s‚ÌUŒ‚‚Í‚»‚ê‚½B ",
					nick_name);
#else
		...
		if (mon_damage(monster, damage)) {	/* still alive? */
			if (!fight_monster) {
#ifdef JAPAN
				sprintf(hit_message, "%s‚ÌUŒ‚‚Í–½’†‚µ‚½B ",
				nick_name);
#else
