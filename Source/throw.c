...
#ifdef JAPAN
	if ((wch = pack_letter("投げる武器は？", WEAPON)) == CANCEL) {
#else
...
object *monster, *weapon;
{
	...
	weapon->quantity = 1;
#ifdef JAPAN
	sprintf(hit_message, "%s", name_of(weapon));
#else
...
short row, col;
{
	...
	if (found || (i == 0)) {
		...
		*new_weapon = *weapon;
		...
		weapon->quantity = 1;
#ifdef JAPAN
		sprintf(msg, "%sは地面に落ちると、どこかに消えてしまった。",
#else
...
{
#ifdef  OS2EMX
        /* EMX fails changin static data */
        static char *pos_init = "\010\007\001\003\004\005\002\006\0";
	static char pos[9] ;
#else
